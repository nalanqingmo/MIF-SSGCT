...
class STG_Co(nn.Module):
    def __init__(self, opt, dim, dataset):
        super(STG_Co, self).__init__()
        self.dataset = dataset
        self.dim = dim
        self.fc1 = nn.Sequential(nn.Linear(dim, dim), nn.LeakyReLU(), nn.Linear(dim, dim))
        self.fc2 = nn.Sequential(nn.Linear(dim, dim), nn.LeakyReLU(), nn.Linear(dim, dim))
    ...
    def forward(self, z1, z2):
        ...
        def co_loss(h1, h2):
            pos = score(h1, h2)
            neg1 = score(h1, row_column_shuffle(h2))
            one = torch.tensor(neg1.shape[0], dtype=torch.float32).to(neg1.device)
            sub_loss = torch.sum(-torch.log(1e-8 + torch.sigmoid(pos)) - torch.log(1e-8 + (one - torch.sigmoid(neg1))))
            return sub_loss

        con_loss = 0
        for w in range(self.n_time):
            # Ensure z1 and z2 are tensors
            h1 = self.fc1(z1.permute([1, 0, 2])[w])
            h2 = self.fc2(z2.permute([1, 0, 2])[w])
            sub_loss = co_loss(h1, h2)
            con_loss += sub_loss * self.decay**(w-1)
        return con_loss * self.opt.l2

class CombineGraph(Module):
    def __init__(self, opt, num_node, adj_all,num,dataset):
        ...
        self.STG_Co = STG_Co(opt,self.dim,self.dataset)
    ...
    def forward(self, inputs, adj, adj1, mask_item, item):
        ...
        co_loss = self.STG_Co( h_local1, h_local1)
        ...
        return output, h_local1,co_loss
...
def forward(model, data):
    ...
    hidden, hidden1,con_loss = model(items, adj, adj1, mask, inputs)
    ...
    return targets, model.compute_scores(seq_hidden, seq_hidden1, mask),con_loss
